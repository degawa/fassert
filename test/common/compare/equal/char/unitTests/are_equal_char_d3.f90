module test_common_compare_equal_char_unitTests_areEqual_D3
    ...
    implicit none
    private
    public :: areEqual_D3_parameterized_test

contains
    subroutine areEqual_D3_parameterized_test(error)
        implicit none
    ...
    end subroutine areEqual_D3_parameterized_test
    ...
    subroutine run_test_cases(spec, results)
        ...
        character(16) :: val1(2, 1, 1), val2(2, 1, 1)
        ...
        block
            ...
            do case = 1, results%get_number_of_test_cases()
                ...
                allocate (character(len(extract(val1(1, 1, 1)))) :: lhs(size(val1, dim=1), &
                                                                        size(val1, dim=2), &
                                                                        size(val1, dim=3)))
                allocate (character(len(extract(val2(1, 1, 1)))) :: rhs(size(val2, dim=1), &
                                                                        size(val2, dim=2), &
                                                                        size(val2, dim=3)))
                do i = 1, size(val1, dim=1)
                    lhs(i, 1, 1) = extract(val1(i, 1, 1))
                    rhs(i, 1, 1) = extract(val2(i, 1, 1))
                end do
...
end module test_common_compare_equal_char_unitTests_areEqual_D3
