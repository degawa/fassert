module test_common_compare_equal_char_unitTests_areEqual_D1
    ...
    implicit none
    private
    public :: areEqual_D1_parameterized_test

contains
    subroutine areEqual_D1_parameterized_test(error)
        implicit none
    ...
    end subroutine areEqual_D1_parameterized_test
    ...
    subroutine run_test_cases(spec, results)
        ...
        block
            ...
            do case = 1, results%get_number_of_test_cases()
                ...
                allocate (character(len(extract(val1(1)))) :: lhs(size(val1)))
                allocate (character(len(extract(val2(1)))) :: rhs(size(val2)))
                do i = 1, size(val1)
                    lhs(i) = extract(val1(i))
                    rhs(i) = extract(val2(i))
                end do
...
end module test_common_compare_equal_char_unitTests_areEqual_D1
