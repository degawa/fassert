module test_sameShape_expectSameShape_unitTests_expect_D3
    ...
    private
    ! D: dimension
    public :: expectSameShepeD3_parameterized_test

contains
    subroutine expectSameShepeD3_parameterized_test(error)
        use :: par_funnel
        implicit none
        ...
        params = [( &
                  ...
                  new_test_parameter(arguments="vtype='"//vtype_list(i)%val//"' dim_act=3,2,1 dim_exp=3,2,1 "// &
                  ...
                  , new_test_parameter(arguments="vtype='"//vtype_list(i)%val//"' dim_act=3,2,1 dim_exp=3,2,1 "// &
                                       ...
                                       expected="message='    Expected Shape: (3,2,1)\n"// &
                                       "    Actual Shape  : (3,2,1)' stat_exp=true") &
                  !v failed case
                  !-v expect_same_shape(actual,expected,name,stat)
                  , new_test_parameter(arguments="vtype='"//vtype_list(i)%val//"' dim_act=3,2,1 dim_exp=1,2,3 "// &
                                       "test_name='a unit test'", &
                  ...
                  , new_test_parameter(arguments="vtype='"//vtype_list(i)%val//"' dim_act=3,2,1 dim_exp=1,2,3 "// &
                                       "test_name='a unit test' verbose=true quiet=true", &
                                       expected="message='    Expected Shape: (1,2,3)\n"// &
                                       "    Actual Shape  : (3,2,1)' stat_exp=false") &
                  , i=1, size(vtype_list)) &
                  ]
    ...
    contains
        subroutine run_test_cases(params, results)
            ...
            ! arguments
            character(64) :: vtype
            integer(int32) :: dim_act(3), dim_exp(3)
            ...
            block
                ...
                do case = 1, results%get_number_of_test_cases()
                    ...
                    case ("int32")
                        block
                            allocate (integer(int32) :: act(dim_act(1), dim_act(2), dim_act(3)))
                            allocate (integer(int32) :: exp(dim_exp(1), dim_exp(2), dim_exp(3)))
                    ...
                    case ("real32")
                        block
                            allocate (real(real32) :: act(dim_act(1), dim_act(2), dim_act(3)))
                            allocate (real(real32) :: exp(dim_exp(1), dim_exp(2), dim_exp(3)))
                    ...
                    case ("real64")
                        block
                            allocate (real(real64) :: act(dim_act(1), dim_act(2), dim_act(3)))
                            allocate (real(real64) :: exp(dim_exp(1), dim_exp(2), dim_exp(3)))
        ...
        subroutine setup_case(param, unit_number, case_name, arg_pres, &
            ...
            integer(int32), intent(out) :: dim_act(3), dim_exp(3)
    ...
    end subroutine expectSameShepeD3_parameterized_test
end module test_sameShape_expectSameShape_unitTests_expect_D3
