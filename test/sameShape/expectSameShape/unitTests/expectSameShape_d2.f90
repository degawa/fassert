...
contains
    subroutine expectSameShepeD2_parameterized_test(error)
        ...
        params = [( &
                  ...
                  new_test_parameter(arguments="vtype='"//vtype_list(i)%val//"' dim_act=3,2 dim_exp=3,2 "// &
                  ...
                  , new_test_parameter(arguments="vtype='"//vtype_list(i)%val//"' dim_act=3,2 dim_exp=3,2 "// &
                                       ...
                                       expected="message='    Expected Shape: (3,2)\n"// &
                                       "    Actual Shape  : (3,2)' stat_exp=true") &
                  !v failed case
                  !-v expect_same_shape(actual,expected,name,stat)
                  , new_test_parameter(arguments="vtype='"//vtype_list(i)%val//"' dim_act=3,2 dim_exp=2,3 "// &
                                       "test_name='a unit test'", &
                  ...
                  , new_test_parameter(arguments="vtype='"//vtype_list(i)%val//"' dim_act=3,2 dim_exp=2,3 "// &
                                       "test_name='a unit test' verbose=true quiet=true", &
                                       expected="message='    Expected Shape: (2,3)\n"// &
                                       "    Actual Shape  : (3,2)' stat_exp=false") &
                  , i=1, size(vtype_list)) &
                  ]
    ...
    contains
        ...
        subroutine setup_case(param, unit_number, case_name, arg_pres, &
            ...
            integer(int32), intent(out) :: dim_act(2), dim_exp(2)
