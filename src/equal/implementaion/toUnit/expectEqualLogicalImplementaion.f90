...
contains
    ...
    !>`quiet`が真の場合，成功時の出力を抑制する．
    !>
    module procedure expect_equiv_logical_rank1
        use :: fassert_common_check
        use :: expectSameShape
        ...
        if (is_verbose_output(stat, verbose, quiet)) call output_on_failure(actual, expected)
    end procedure expect_equiv_logical_rank1
    ...
    !>`quiet`が真の場合，成功時の出力を抑制する．
    !>
    module procedure expect_equiv_logical_rank2
        use :: fassert_common_check
        use :: expectSameShape
        use :: fassert_common_compare_equal_array
        implicit none
        ...
        if (is_verbose_output(stat, verbose, quiet)) call output_on_failure(actual, expected)
    end procedure expect_equiv_logical_rank2
    ...
    !>`quiet`が真の場合，成功時の出力を抑制する．
    !>
    module procedure expect_equiv_logical_rank3
        use :: fassert_common_check
        use :: expectSameShape
        use :: fassert_common_compare_equal_array
        implicit none
        ...
        if (is_verbose_output(stat, verbose, quiet)) call output_on_failure(actual, expected)
    end procedure expect_equiv_logical_rank3
