...
contains
    ...
    pure function is_approx_equal_real32(val1, val2, tolerance) result(are_same)
        ...
        are_same = (abs(val1 - val2) <= optval(tolerance, &
                                               default=epsilon(val1)))
    end function is_approx_equal_real32

    !>二つのスカラ値の差が許容値以下の場合に`.true.`，そうでない場合`.false.`を返す．
    !>許容値が指定されない場合，マシンイプシロンが許容値として使われる．
    pure function is_approx_equal_real64(val1, val2, tolerance) result(are_same)
        use :: assert_common_optval
        implicit none
